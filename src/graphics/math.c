...
/*   Updated: 2024/07/01 18:04:44 by wil              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    wall_v_hit(t_graph *g, t_ray *r)
{
	...
	// ft_printf("last Vertical hit X : %.2f Y : %.2f L: %.2f\n", r->wvhx, r->wvhy, r->wvhl);
}
...
void	wall_h_hit(t_graph *g, t_ray *r)
{
	...
	// ft_printf("last Horizontal hit X : %.2f Y : %.2f L: %.2f\n", r->whhx, r->whhy, r->whhl);
}
...
/*	ft_printf("Pos X : %.2f Y : %.2f\n", r->pos_x, r->pos_y);
	ft_printf("Vertical hit X : %.2f Y : %.2f\n", r->fvhx, r->fvhy);
	ft_printf("Horizontal hit X : %.2f Y : %.2f\n", r->fhhx, r->fhhy);
*/
}
...
void    loop_rays(t_graph *g)
{
	...
	while (i < WIN_X)
    {
		...
		//ft_printf("Side %d, Offset: %.2f, SOW: %.2f\n", g->ray.soi, g->ray.ooi, g->ray.sow);
		draw_texture(g, i++, g->ray);
		mlx_put_image_to_window(g->mlx, g->win, g->i.img, 1, 1);
